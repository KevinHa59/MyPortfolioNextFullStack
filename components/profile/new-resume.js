    ...
    <Stack width={"100%"} height={"100%"} gap={"1px"} position={"relative"}>
      <Header title={"New Resumes"} icon={<Article />}>
        <Stack direction={"row"} gap={1}>
          <Input
            inputProps={{ startAdornment: <Label sx={{ paddingRight: 1 }} /> }}
      ...
      <Stack direction={"row"} padding={2}>
        ...
        <Stack width={"100%"} alignItems={"center"}>
          ...
          >
            ...
            <Stack id={steps[1].name} width={"100%"} height={"100%"}>
              <Education data={resumeData.education} step={steps[1]} />
            </Stack>
            <Stack id={steps[2].name} width={"100%"} height={"100%"}>
              <Certification data={resumeData.certifications} step={steps[2]} />
            </Stack>
            <Stack id={steps[3].name} width={"100%"} height={"100%"}>
              <Skill data={resumeData.skills} step={steps[3]} />
            </Stack>
            <Stack id={steps[4].name} width={"100%"} height={"100%"}>
              <Project data={resumeData.projects} step={steps[4]} />
            </Stack>
            <Stack id={steps[5].name} width={"100%"} height={"100%"}>
              <WorkExperience
                data={resumeData.workExperience}
                step={steps[5]}
              />
            </Stack>
            <Stack id={steps[6].name} width={"100%"} height={"100%"}>
              <VolunteerExperience
                data={resumeData.volunteerExperience}
                step={steps[6]}
              />
            </Stack>
            <Stack id={steps[7].name} width={"100%"} height={"100%"}>
              <Award data={resumeData.awards} step={steps[7]} />
            </Stack>
            <Stack id={steps[8].name} width={"100%"} height={"100%"}>
              <Language data={resumeData.languages} step={steps[8]} />
            </Stack>
            <Stack id={steps[9].name} width={"100%"} height={"100%"}>
